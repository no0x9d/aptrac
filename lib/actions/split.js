...
module.exports = function split(context, task, done) {
    ...
    // set date to task date when option at date is current date and task is on another day<
    if(!task.start.isSame(options.at, 'day') && options.at.isSame(new Date(),'day')) {
        options.at.dayOfYear(task.start.dayOfYear());
    }
