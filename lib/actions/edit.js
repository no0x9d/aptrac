    ...
    if (query) {
        ...
        find(args, query, function (err, context, tasks) {
            ...
            if (changes.start || changes.end) {
                ...
                find(args, conflictQuery, function (err, context, tasks) {
                    if (err) {
                        return done(err);
                    }
                    ...
                    if (completeOverlap.length > 0 || startOverlap.length > 1 || endOverlap.length > 1) {
                        done(new OverlappingTaskError("Fatal error while editing task! Can't apply changes", task, conflictingTasks))
                    }

                    if (startOverlap.length === 1 || endOverlap.length === 1) {
                        return done(new OverlappingTaskError("Error while editing task! Please update conflicting tasks first.", task, conflictingTasks))
                    }
