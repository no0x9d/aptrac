...
function addOptionalFields(taskItem, fields, context) {
    "use strict";
    var options = context.options;

    for (var fieldName in schema) { //FIXME: should take all fields from schema definition and do formatting
        if (schema.hasOwnProperty(fieldName)) {
            if (fields[fieldName] || !(options && options.hideEmpty)) {
                taskItem[schema[fieldName].label] = fields[fieldName];
            }
        }
    }
}
...
function outputDays(context, groupedTasks) {
    ...
    for (var group in groupedTasks) {
        ...
        if (options && options.condense) {
            ...
            var groupedbyTask = tasks.groupBy(function (task) { //FIXME: must support for current schema
                return (task.task || '')
                    + (task.project || '')
                    + (task.note || '');
            });
            ...
            for (var groupedTask in groupedbyTask) {
                ...
                addOptionalFields(taskItem, groupedTaskRepresentant, context);
            ...
            tasks.forEach(function (task) {
                ...
                addOptionalFields(taskItem, task, context);
